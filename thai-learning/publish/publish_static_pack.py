...
def _validated_lesson(lesson: dict[str, Any]) -> tuple[str, str, int]:
    ...
    if not isinstance(voices, dict) or voices.get("provider") != "azure_speech":
        raise PublishError("Lesson voices must use azure_speech")
    for voice_name in ("thai", "hebrew"):
        if not isinstance(voices.get(voice_name), str) or not voices[voice_name].strip():
            raise PublishError(f"Lesson {voice_name} voice must be a non-empty string")
    ...
    if audio_program.get("normal_speed_only") is not True:
        raise PublishError("Lesson audio_program must be normal-speed-only")
...
def _validated_audio(
    ...
    for track in manifest_tracks:
        ...
        expected_request_hash = audio.request_sha256(
            audio.build_track_ssml(lesson, lesson_tracks[track_id])
        )
...
def _require_identical_pack(
    ...
    try:
        ...
        actual_names = {item.name for item in pack_directory.iterdir()}
